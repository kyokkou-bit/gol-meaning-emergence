...
def build_osc_environment(rng: np.random.Generator) -> np.ndarray:
    while True:
...
def simulate_condition(g_ext: float, rng: np.random.Generator) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    for trial in range(1, N_TRIALS + 1):
        env_grid = build_osc_environment(rng)
...
def save_outputs(
    ...
    grid_csv = ROOT / "gol_ring_external_osc_env_trials.csv"
    trial_csv = ROOT / "gol_ring_external_osc_env_clusters.csv"
    structure_csv = ROOT / "gol_ring_external_osc_env_structure_summary.csv"
    a_mode_csv = ROOT / "gol_ring_external_osc_env_A_mode_summary.csv"
    osc_pos_csv = ROOT / "gol_ring_external_osc_env_osc_position_summary.csv"
    report_md = ROOT / "gol_ring_external_osc_env_report.md"
    ...
    lines = [
        "# ring_1d + external osc environment scan",
        "",
        "- Internal topology: A-B-C-D-A ring with weak-average coupling.",
        "- External node E is a fixed osc-environment template regenerated per trial and coupled one-way to A only.",
